...
const Producto = () => {
    ...
    useEffect(() => {
        if(id && consultarDB) {
            const obtenerProducto = async () => {
                const productoQuery = await firebase.db.collection('aplicaciones').doc(id);
    ...
    const votarProducto = () => {
        ...
        //  Actualizar en la BD
        firebase.db.collection('aplicaciones').doc(id).update({ 
            votos: nuevoTotal, 
    ...
    const eliminarProducto = async () => {
        ...
        try {
            await firebase.db.collection('aplicaciones').doc(id).delete();
            router.push('/')
        } catch (error) {
            console.log(error);
        }
                                ...
                                {comentarios.length === 0 
                                    ...
                                    : ( <ul>
                                        {comentarios.map((comentario, i) => (
                                            <li key={`${comentario.usuarioId}-${i}`} css={css`border: 1px solid #e1e1e1;padding: 2rem; `} >
